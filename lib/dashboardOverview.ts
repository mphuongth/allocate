// The dashboard overview computation — every figure the app derives from a
// user's holdings (net worth, per-goal totals, allocation by type, insurance).
//
// It lives here rather than inside GET /api/v1/dashboard/overview because a
// second server-side consumer needs the same numbers: the PDF report route used
// to render whatever DashboardData the *client* posted, so an authenticated user
// could forge the figures in their own report and submit arbitrarily large or
// deep payloads for the server to render (#594). Both routes now call this
...
/**
 * `ok: false` means at least one required read failed. Callers must NOT fall
 * back to a partial figure — see the required-error gate below: an understated
 * total would also be persisted into net_worth_snapshots (#513).
 */
...
/**
 * Compute the dashboard overview for one user. The caller owns authentication —
 * pass the *authenticated* user's id and their own request-scoped client, so RLS
 * applies exactly as it does for the dashboard endpoint.
 *
 * Side effect: upserts today's net-worth snapshot when the rounded total changed
 * (see the end of the function). That is a property of "the total was computed
 * today", not of which endpoint asked, so it stays here.
 */
export async function buildDashboardOverview(
  ...
  const [insExclusionsRes, insOverridesRes, recOverridesRes, recFulfillmentsRes] = await Promise.all([
    ...
    supabase.from('recurring_saving_fulfillments').select('recurring_saving_id, ym').eq('user_id', userId),
  ])
  ...
  if (shouldWriteSnapshot(snapshotRes.data, totalAssets)) {
    const written = await persistSnapshot(() =>
      supabase.from('net_worth_snapshots').upsert(
        { user_id: userId, snapshot_date: today, total_assets: Math.round(totalAssets) },
        { onConflict: 'user_id,snapshot_date' }
      )
    )
    if (!written.ok) {
      // Context only — never the balance itself.
      console.error('[dashboardOverview] net-worth snapshot write failed', {
        user_id: userId,
        snapshot_date: today,
  ...
  return { ok: true, data: {
    ...
    // Spelled out rather than passed through: the accumulator is indexed by the
    // raw `asset_type` string, so only the three buckets the dashboard defines
    // can reach the payload (nothing else is ever added — see the `in` guard).
