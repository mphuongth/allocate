-- Rename a bank-named deposit whenever the money moves banks (#640 follow-up).
--
-- 20260807000001 renamed the deposit only when its `notes` matched the OLD
-- bank's reference name EXACTLY. Two ordinary deposits slip through that test and
-- keep advertising a bank they are no longer held at:
--
--   • capitalisation drift — a real deposit reads "PVCombank" while the reference
--     list says "PVcomBank", so a case-sensitive `=` never fired;
--   • deposits older than the structured bank field — `bank_code` is null, so
--     there was no old name to compare against at all, even though the label is
--     plainly a bank's name.
--
-- The test is now "is this label a bank name?" rather than "is it THIS bank's
-- name?": if the notes match any row in `public.banks` (case-insensitively,
-- trimmed), the label is bank-derived and follows the money. Anything else —
-- "Sổ cưới", "Tiết kiệm mua nhà" — is the user's own name and is left alone,
-- which is the property that matters. The closed-cycle snapshot is still built
-- from the pre-update row, so history keeps the bank the money was held at.
--
-- Body is otherwise identical to 20260807000001. Forward migration, unchanged
-- signature, so create-or-replace needs no DROP.
